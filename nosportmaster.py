...
parser = argparse.ArgumentParser(description='TurnOn ports on Cisco devices.')
...
parser.add_argument('--NOT-DRY-RUN', action='store_true', help='Perform the TurnOn operation (default is dry run).')
parser.add_argument('--sleep-time', type=int, default=2, help='Specify sleep time between operations (in ms).') 
parser.add_argument('--log-file', type=str, default='cisco_device_TurnOn.log', help='Specify the log file name.')
...
def turn_on_the_port(ip_address_of_device, port_name, username, password, dry_run=True):
    """Connect to a Cisco device and turn on the specified port.
    
    Args:
        ip_address_of_device (str): The IP address of the Cisco device.
        port_name (str): The name of the port to be turn on.
        username (str): The username for SSH authentication.
        password (str): The password for SSH authentication.
        dry_run (bool): If True, perform a dry run without turnin on the port.
    """
    try:
        ...
        if dry_run:  # Check if it is a dry run
            log_and_print(f"*** Dry run: Not Turn On interface {port_name} on {ip_address_of_device}. ***")  # Log dry run message
        else:  # If not a dry run
            log_and_print(f"Turn ON interface {port_name} on {ip_address_of_device}...")  # Log turn on interface
            remote_connection.send('no shutdown\n')  # Send command to turn on the interface
            time.sleep(2)  # Wait for command to execute
...
def turn_on_ports_on_multiple_devices(devices, username, password, dry_run=True):
    """turn on ports on multiple Cisco devices, or perform a dry run.
    
    Args:
        devices (list): List of devices to configure.
        username (str): The username for SSH authentication.
        password (str): The password for SSH authentication.
        dry_run (bool): If True, perform a dry run without turning on ports.
    """
    for device in devices:  # Loop through each device
        ...
        turn_on_the_port(ip, port_name, username, password, dry_run)  # Call the function to turn on the port
        log_and_print(f"\n--- End of {device['ip']} ---")

def display_manual():
    """Display the manual for using the script."""
    manual_text = """
    Cisco Device Port TurnOn Script Manual
    -----------------------------------------
    
    This script allows you to connect to Cisco devices and turn on specific ports. It can perform a dry run 
    ...
    Important Notes:
    -----------------
    - Ensure that you have the necessary permissions to turn on interfaces on the devices.
    ...
    Logs:
    ------
    All actions taken by the script are logged in a file named 'cisco_device_TurnOn.log'.
    This log file contains timestamps and detailed messages about the operations performed.

    Example Usage:
    ---------------
    To turn on ports without making changes (dry run):
...
def main():
    ...
    # Execute the TurnOn sequence
    turn_on_ports_on_multiple_devices(devices, username, password, dry_run=dry_run)  # Call function to turn on ports
